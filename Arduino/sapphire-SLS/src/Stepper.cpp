...
double Stepper_Acceleration_Step[3] = {
...
void StepperA_StartMove(float dis) {
	...
	IsMoving_A = true;
	STEPPER_A_TIMER.attachInterrupt(StepperA_Moving);
	STEPPER_A_TIMER.setPeriod(Stepper_Delay_Period[0]);
	STEPPER_A_TIMER.start();
...
void StepperB_StartMove(float dis) {
	...
	IsMoving_B = true;
	STEPPER_B_TIMER.attachInterrupt(StepperB_Moving);
	STEPPER_B_TIMER.setPeriod(Stepper_Delay_Period[1]);
	STEPPER_B_TIMER.start();
...
void StepperC_StartMove(float dis) {
	...
	IsMoving_C = true;
	STEPPER_C_TIMER.attachInterrupt(StepperC_Moving);
	STEPPER_C_TIMER.setPeriod(Stepper_Delay_Period[2]);
	STEPPER_C_TIMER.start();
